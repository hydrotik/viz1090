...
- Current country-scale raster plan: use `tools/build_map_tile_pack.py` and `tools/coverage_profiles.py`, not one giant high-zoom national file. `conus-overview` covers lower-48 zoom 0-7; regional packs (`northeast`, `southeast`, `midwest`, `south-central`, `mountain-west`, `west-coast`) cover regional zoom 0-12; `nyc` remains zoom 0-14. Higher-detail travel packs include `west-coast-hd` and `mountain-west-hd` at zoom 0-14, `arizona-hd` at zoom 0-14, `tri-state-ultra` at zoom 0-15, and `nyc-west-li-ultra` at zoom 0-16 for NYC/western Long Island. `run_uconsole_station.sh --map-tile-profile auto --weather-profile regional` selects the best installed raster pack and falls back to generated vector maps when no pack exists. `deploy_uconsole.sh --tiles starter` syncs code plus available starter packs; `--tiles conus-regions` syncs the lower-48 overview plus regional packs; `--tiles travel-hd` syncs the high-detail western/Arizona/tri-state packs when present; `--tiles focused-hd` is the storage-conscious travel set that keeps NYC/western Long Island, tri-state fallback, western/AZ HD packs, and skips lower-priority Midwest/Great Lakes/South Central style packs.
...
- FLOCK color semantics: the data still classifies `kind: 0=surveillance, 1=ALPR, 2=Flock Safety`, but the renderer intentionally uses one red family instead of blue/magenta/yellow so camera points are recognizable without being confused for aircraft on the dark raster basemap. Direction wedges are drawn brighter than the core dot when heading is known. Aircraft draw after radar and FLOCK.
- Off-screen aircraft: active tracked aircraft outside the current viewport draw an edge-docked radar-style tick instead of a center-origin bearing line or dot. Left/right off-screen traffic uses a vertical edge tick; above/below traffic uses a horizontal edge tick. Tick length grows modestly with off-screen distance, and the marker stays docked to the edge until pan/zoom brings the aircraft into view. A thinner parallel echo tick animates inward from the edge and fades out. Off-screen aircraft labels/tooltips are intentionally hidden, and bottom/side ticks are clamped above the status strip.
- uConsole controls: left/right pan behavior is intentionally reversed from the original SDL motion mapping and currently feels correct on device. Up/down pan is also inverted from the original mapping so pressing up/down moves the map in the user's expected direction.
